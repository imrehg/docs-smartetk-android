...
.. java:method:: int setEnable(boolean enable)
   ...
   :return: ``S_OK`` if function succeeds, ``E_*`` otherwise

.. java:method:: int setDirection(int direction)

   Set input/output direction for the specific GPIO pin. 

   :param int direction:  ``GPIO.GM_GPI`` for input direction, ``GPIO.GM_GPO`` for output direction. 
   :returns: ``S_OK`` if function succeeds, ``E_*`` otherwise

.. java:method:: int getDirection(int[] direction)

   Get direction state of the specific GPIO Pin.

   :param int[] direction: parameter to set to ``GPIO.GM_GPI`` for input direction, or ``GPIO.GM_GPO`` for output direction
   :return: ``S_OK`` if the function succeeded, ``E_*`` otherwise
